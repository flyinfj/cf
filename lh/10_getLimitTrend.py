...
model = myLib.MyLib()
# 1. 从days_limitup_pre文件获取最近10天数据
start_date = model.get_trade_dates(10,0).iloc[0,0]
limitup_df = tools.Tools().db_query(f'select * from days_limitup_pre where trade_date > \'{start_date}\' order by trade_date')
...
# 2. 加载stock_spot数据
stock_spot_df = tools.Tools().db_query(f'select * from stock_spot where trade_date >= now() - interval 90 day order by trade_date')
column_mapping = {'stock_code': '代码','trade_date': '时间','close_price': '最新价'}
stock_spot_df = stock_spot_df.rename(columns=column_mapping)
stock_spot_df['最新价'] = pd.to_numeric(stock_spot_df['最新价'], errors='coerce')

# 3. 获取最后15行数据并转换格式
result_dfs = []
for code in limitup_df['代码'].unique():
    limitup_date = limitup_df[limitup_df['代码'] == code]['时间'].iloc[0]
    limitup_date = pd.to_datetime(str(limitup_date), format='%Y%m%d', errors='coerce')
    code_data = stock_spot_df[stock_spot_df['代码'] == code].tail(15)
    code_data['时间'] = pd.to_datetime(code_data['时间'].astype(str), format='%Y%m%d')
    code_data['最新价'] = pd.to_numeric(code_data['最新价'], errors='coerce')
    code_data = code_data[code_data['时间'] >= limitup_date]
    prices = code_data['最新价'].to_numpy(dtype=float)
    if len(prices) > 0:
