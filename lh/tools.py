...
class Tools:
    ...
    @staticmethod
    def normalize_http_value(value: str):
        if value is None:
            return ''
    ...
    def eastmoney_curl(url: str,data:dict = None):
        ...
        cmd = ['curl', '-x', '175.178.27.33:3128', url]
    ...
    def eastmoney_curlget(url: str):
        url = Tools.normalize_http_value(url)
        headers = {
        ...
        for _ in range(3):
            ...
            for k, v in headers.items():
                header_value = Tools.normalize_http_value(v)
                cmd += ['-H', f'{k}: {header_value}']
            print(cmd)
            res = subprocess.run(cmd, capture_output=True, text=True)
        ...
        if res is None or res.returncode != 0 or not res.stdout.strip():
            raise RuntimeError(f'eastmoney_curlget failed after retries: {url}; {last_error}')
