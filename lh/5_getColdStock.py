        ...
        if response.status_code == 200:
            ...
            if 'result' in data and 'data' in data['result']:
                ...
                security_codes = [{'代码': item['SECURITY_CODE'],
                                   ...
                                   '涨幅': item['CHANGE_RATE'],
                                   '热榜': item['POPULARITY_RANK'],
                                   '换手': item['TURNOVERRATE'],
                                   '量比': item['VOLUME_RATIO'],
                                   '成交额': round(item['DEAL_AMOUNT']/100000000,2)}
                                  for item in stock_data]
                df = pd.DataFrame(security_codes, columns=['代码', '名称', '涨幅', '热榜', '换手', '量比', '成交额'])
                return df
            else:
                print("未找到股票信息")
        else:
...
if current_time >= datetime.datetime.strptime('10:00:00', '%H:%M:%S').time():
    ...
    if not df.empty:
        ...
        nineturn_df = nineturn_df.drop(columns=['成交额'])
        # 获取标签
