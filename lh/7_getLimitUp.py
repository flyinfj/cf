...
def get_stock_his(stock_code, klt,start_date):
    # 构造请求URL
    return myLib.MyLib().get_stock_his(stock_code, klt,start_date)
...
column_mapping = {'id': '序号', 'stock_code': '代码', 'stock_name': '名称', 'change_percent': '涨跌幅', 'last_price': '最新价', 'turnover_amount': '成交额', 'circul_marketcap': '流通市值', 'total_marketcap': '总市值', 'turnover_rate': '换手率', 'limitup_amount': '封板资金', 'first_limitup_time': '首次封板时间', 'last_limitup_time': '最后封板时间', 'breakout_count': '炸板次数', 'limitup_stats': '涨停统计', 'limitup_days': '连板数', 'industry': '所属行业', 'trade_date': 'date'}
all_limitup = all_limitup.rename(columns=column_mapping)
...
for index, row in all_limitup_num.iterrows():
    ...
    limitup_df = tools.Tools().db_query(f'select * from stock_spot where stock_code = \'{code}\' and trade_date = \'{limitup_date}\'')
    column_mapping = {'stock_code': '代码','trade_date': '时间','close_price': '最新价'}
    limitup_df = limitup_df.rename(columns=column_mapping)  

    if not limitup_df.empty:
        limitup_price = limitup_df['最新价'].values[0]
        c_df = tools.Tools().db_query(f'select * from stock_spot \
            where stock_code = \'{code}\' order by trade_date desc limit 1')
        c = c_df.iloc[-1]['close_price']
...
if stock_codes:
    ...
    sorted_df['涨<1量'] = sorted_df['涨<1量'].apply(lambda x: f"{x}%")
    sorted_df['涨>3量'] = sorted_df['涨>3量'].apply(lambda x: f"{x}%")
    sorted_df['涨>2量'] = sorted_df['涨>2量'].apply(lambda x: f"{x}%")
    sorted_df['涨>1量'] = sorted_df['涨>1量'].apply(lambda x: f"{x}%")
    print(model.beautify(sorted_df))
    model.output_file(sorted_df,'涨幅后出货偏少个股')
