...
def get_limitup_sector():
    ...
    if 'result' in data and 'data' in data['result']:
        stock_data = data['result']['data']
        security_codes = [
            {'板一/板数/高度': f'{stock_data[0]['BOARD_NAME']}/{stock_data[0]['HLIMITE_NUM']}/{stock_data[0]['STOCK_HIGH']}',
             '板二/板数/高度': f'{stock_data[1]['BOARD_NAME']}/{stock_data[1]['HLIMITE_NUM']}/{stock_data[1]['STOCK_HIGH']}',
             '板三/板数/高度': f'{stock_data[2]['BOARD_NAME']}/{stock_data[2]['HLIMITE_NUM']}/{stock_data[2]['STOCK_HIGH']}',
             '板四/板数/高度': f'{stock_data[3]['BOARD_NAME']}/{stock_data[3]['HLIMITE_NUM']}/{stock_data[3]['STOCK_HIGH']}',
             '板五/板数/高度': f'{stock_data[4]['BOARD_NAME']}/{stock_data[4]['HLIMITE_NUM']}/{stock_data[4]['STOCK_HIGH']}'
             }]
        df = pd.DataFrame(security_codes, columns=['板一/板数/高度', '板二/板数/高度', '板三/板数/高度', '板四/板数/高度', '板五/板数/高度'])
        return df
    else:
        print("未找到股票信息")
...
def get_vol_rate(stock_code):
    ...
    # 获取近两天的 5 分钟行情信息
    stock_his = myLib.MyLib().get_stock_his_tx(stock_code, 5, '',2)  # 返回 DataFrame，包含 DATE, PRICE, VOL
    total_yes = 0
    if not stock_his.empty:
        stock_his['时间'] = pd.to_datetime(stock_his['时间'])
        stock_his['成交量'] = stock_his['成交量'].astype(int)
        ...
        if not stock_tod_df.empty:
            # 计算昨日成交总量 TOTAL_YES
            total_yes =stock_yes_df['成交量'].sum()
            # 获取当前价格（假设当前价格为今天的最后一笔成交价）
            current_price = stock_tod_df['收盘'].iloc[-1]
            # 计算昨天套牢量 TL_YES
            tl_yes = stock_yes_df[(stock_yes_df['收盘'] > current_price)]['成交量'].sum()
            # 计算昨天获利量 HL_YES
            hl_yes = stock_yes_df[(stock_yes_df['收盘'] <= current_price)]['成交量'].sum()
            # 计算今天套牢量 TL_TOD
            tl_tod = stock_tod_df[(stock_tod_df['收盘'] >= current_price * 1.02)]['成交量'].sum()
            # 计算今天获利量 HL_TOD
            hl_tod = stock_tod_df[(stock_tod_df['收盘'] <= current_price * 0.98)]['成交量'].sum()
            # 计算今天正常量 NO_TOD
            total_tod = stock_tod_df['成交量'].sum()
...
def calc_alert_days(stock_codes):
    start_time = myLib.MyLib().get_trade_dates(32,0)
    ...
    for stock_code in stock_codes:
        # 过滤出对应的股票数据
        stock_data = tools.Tools().db_query(f'select * from stock_spot where stock_code = \'{stock_code}\' and trade_date >= now() - interval 60 day order by trade_date')
        column_mapping = {'stock_code': '代码','trade_date': '时间','close_price': '最新价'}
        stock_data = stock_data.rename(columns=column_mapping)

        required_columns = {'时间', '最新价'}
        if stock_data.empty or not required_columns.issubset(stock_data.columns):
            result.append({
                '代码': stock_code,
                '警告': '无行情数据'
            })
        ...
        if df.empty or df['最新价'].dropna().empty:
            result.append({
                '代码': stock_code,
                '警告': f'无有效价格/{max_date}'
            })
            continue
        # 初始化变量
        prices = [float(x) if pd.notna(x) else 0.0 for x in df['最新价'].iloc[-31:].tolist()]
        if stock_code.startswith(('60', '68')):
            prices_z = [float(x) if pd.notna(x) else 0.0 for x in df_z1['收盘'].iloc[-31:].tolist()]
        elif stock_code.startswith('00'):
            prices_z = [float(x) if pd.notna(x) else 0.0 for x in df_z2['收盘'].iloc[-31:].tolist()]
        elif stock_code.startswith('30'):
            prices_z = [float(x) if pd.notna(x) else 0.0 for x in df_z3['收盘'].iloc[-31:].tolist()]
        else:
            prices_z = [float(x) if pd.notna(x) else 0.0 for x in df_z1['收盘'].iloc[-31:].tolist()]
        days = 0  # 计算天数
        ...
        while days < 20:
            # 判断是否触发10天规则
            if alert_days_10 is None and len(prices) >= 11 and prices[-11]>0:
                for i in range(8,11):
                    if prices[-i] == 0:
                        continue
                    ...
                    if deviation_10-deviation_10_z >= 100:
                        ...
                        break
            # 判断是否触发30天规则
            if alert_days_30 is None and len(prices) >= 31 and prices[-31]>0:
...
print(df)
model.output_file(df,'涨停股统计分析',0)
...
model.output_file(model.beautify(df),'龙头股核心信息',0)

df = get_limitup_cons()
df = df.sort_values(by='昨板数', ascending=False)
df.reset_index(drop=True, inplace=True)
print(model.beautify(df))
model.output_file(model.beautify(df),'连板天梯全景',0)
...
#获取昨天的的涨停数据到数据库中
df = tools.Tools().db_query('SELECT max(trade_date) FROM days_limitup_pre')
...
for index, row in filter_datas.iterrows():
    ...
    if df is not None:
        column_mapping = {'时间': 'trade_date', '代码': 'stock_code', '名称': 'stock_name', '昨板数': 'pre_limit_days', '今板数': 'cur_limit_days', '涨幅': 'change_percent', '是否涨停': 'is_limitup'}
        df = df.rename(columns=column_mapping)
        tools.Tools().db_upsert(df,'days_limitup_pre','trade_date','stock_code')
#统计涨停次数
df = tools.Tools().db_query('SELECT * FROM days_limitup_pre WHERE trade_date > NOW() - INTERVAL 180 DAY order by trade_date')
column_mapping = {'trade_date': '时间', 'stock_code': '代码', 'stock_name': '名称', 'pre_limit_days': '昨板数', 'cur_limit_days': '今板数', 'change_percent': '涨幅', 'is_limitup': '是否涨停'}
df = df.rename(columns=column_mapping)
...
count_df['平均涨幅'] = (count_df['平均涨幅']).round(1)
count_df = count_df.sort_values(by='涨停数', ascending=False)
count_df.insert(0, '涨停排序', range(1, len(count_df) + 1))

column_mapping = {'涨停排序': 'rank_id', '代码': 'stock_code', '涨停数': 'limitup_count', '第二天涨次数': 'next_limitup_count', '平均涨幅': 'avg_raise_percent'}
count_df = count_df.rename(columns=column_mapping)
tools.Tools().db_upsert(count_df,'days_limitup_count','stock_code','')
