...
df = pd.DataFrame()
for i in range(-4, 3):
    v_start_date = today + timedelta(days=i)
    print(v_start_date)
    v_end_date = today + timedelta(days=i+1)
    df = myClass.get_caibao(v_start_date.strftime("%Y%m%d"), v_end_date.strftime("%Y%m%d"))
    if not df.empty:
    ...
    tools.Tools().db_execute(f"delete from caibao_info where notice_time = '{v_start_date.strftime("%Y-%m-%d")}' and period is null")
    df_db = tools.Tools().db_query(f"select * from caibao_info where notice_time = '{v_start_date.strftime("%Y-%m-%d")}'")
    if not df_db.empty:
    ...
    if not df.empty: 
        ...
        # 限制
        1. 数据缺失处理**：若【contents】中无对应字段（如无季度数据），直接用-1填充
        2. 数值精度：R和P保留整数（单位：亿元），RR和PR保留整数（单位：%），无法获取数据时强制填-1
...
sql = f"""
    ...
    CONCAT(ci.notice_time,'<br>',ci.type) notice_time_type,
    CONCAT(ci.period,'<br>',ci.year,ci.month) period_year_month,
    CONCAT(case when ci.R=-1 then '-' else ci.R end, '<br>', case when ci.P=-1 then '-' else ci.P end) R_P,
    CONCAT(case when ci.RR=-1 then '-' else ci.RR end, '%<br>', case when ci.PR=-1 then '-' else ci.PR end, '%') RR_PR,
    t2.Q_R,
    t4.Y_R,
    ci.content
    FROM caibao_info ci
    ...
    SELECT 
        stock_code,
        CONCAT(
        GROUP_CONCAT(month ORDER BY year DESC, month DESC SEPARATOR ', '), '<br>',
        GROUP_CONCAT(RR ORDER BY year DESC, month DESC SEPARATOR '%, '), '%<br>',
    ...
    FROM (
        ...
        c.year,
        c.month,
        case when c.RR=-1 then '-' else c.RR end RR,
        case when c.PR=-1 then '-' else c.PR end PR
        FROM caibao_info c
        INNER JOIN (
...
#输出各种业绩
sql = f"SELECT stock_code,stock_name,type,case when RR=-1 then '-' else RR end RR, case when PR=-1 then '-' else PR end PR,notice_time,content  FROM caibao_info WHERE type = '业绩预告' and (RR > 0 or PR > 0) AND notice_time >= '{start_date.strftime('%Y-%m-%d')}' order by notice_time"
df = tools.Tools().db_query(sql)
if not df.empty:
    df.rename(columns={'stock_code': '代码'}, inplace=True)
    df.rename(columns={'stock_name': '名称'}, inplace=True)
...
sql = f"SELECT stock_code,stock_name,type,case when RR=-1 then '-' else RR end RR, case when PR=-1 then '-' else PR end PR,notice_time,content  FROM caibao_info WHERE type = '业绩快报' and (RR > 0 or PR > 0) AND notice_time >= '{start_date.strftime('%Y-%m-%d')}' order by notice_time"
df = tools.Tools().db_query(sql)
if not df.empty:
    df.rename(columns={'stock_code': '代码'}, inplace=True)
    df.rename(columns={'stock_name': '名称'}, inplace=True)
