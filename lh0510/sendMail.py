...
with open(dir_html, 'w', encoding='utf-8') as f:
    ...
    for filename in sorted(os.listdir(html_dir)):
        if filename.endswith('.html') and filename != 'dir.html':
            file_path = os.path.join(html_dir, filename)
            f.write(f'<li><a href=http://120.27.198.74:3389/{filename}>{filename}</a></li>')
...
try:
    ...
    print(f"邮件发送失败: {e}")
"""
tools.Tools().db_exec("delete from api_log where create_time < NOW() - INTERVAL 5 DAY;")
tools.Tools().db_exec("delete from caibao_info where STR_TO_DATE(notice_time, '%Y-%m-%d') < NOW() - INTERVAL 500 DAY;")
tools.Tools().db_exec("delete from days_limitup where STR_TO_DATE(trade_date, '%Y%m%d') < NOW() - INTERVAL 500 DAY;")
tools.Tools().db_exec("delete from days_limitup_count  where create_time < NOW() - INTERVAL 500 DAY;")
tools.Tools().db_exec("delete from days_limitup_pre  where STR_TO_DATE(trade_date, '%Y%m%d') < NOW() - INTERVAL 500 DAY;")
tools.Tools().db_exec("delete from quar_holder_info  where create_time < NOW() - INTERVAL 30 DAY;")
tools.Tools().db_exec("delete from stock_pop where STR_TO_DATE(trade_date, '%Y%m%d') < NOW() - INTERVAL 500 DAY;")
tools.Tools().db_exec("delete from stock_spot where STR_TO_DATE(trade_date, '%Y%m%d') < NOW() - INTERVAL 500 DAY;")
tools.Tools().db_exec("delete from stock_stats where create_time < NOW() - INTERVAL 500 DAY;")
tools.Tools().db_exec("delete from tmp_stock_real where STR_TO_DATE(trade_date, '%Y%m%d') < NOW() - INTERVAL 7 DAY;")   
tools.Tools().db_exec("delete from tmp_subject_stock  where STR_TO_DATE(trade_date, '%Y%m%d') < NOW() - INTERVAL 30 DAY;")
tools.Tools().db_exec("delete from trade_date  where STR_TO_DATE(trade_date, '%Y%m%d') < NOW() - INTERVAL 500 DAY;")
tools.Tools().db_exec("delete from yanbao_ind where STR_TO_DATE(report_date, '%Y-%m-%d') < NOW() - INTERVAL 500 DAY;")
tools.Tools().db_exec("delete from yanbao_stock where STR_TO_DATE(report_date, '%Y-%m-%d') < NOW() - INTERVAL 500 DAY;")
