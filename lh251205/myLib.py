...
class MyLib:
    ...
    def request_post(self, url, data, headers):
        # 兼容传入的数据类型（dict/list/str/bytes）并统一为字节
        ...
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            if "gzip" in enc:
                ...
                try:
                    ...
                    # 如果没有 brotli 支持，退回原文解码，通常会失败但不阻塞
                    pass
            # 按响应头的 charset 解码
    ...
    def requests_get(self, url, max_retries: int = 3):
        """
        稳健的 GET 请求：
        - 增加常见网络错误的重试（包含 RemoteDisconnected / 超时 / 连接重置等）
        - 补充常用请求头，避免被服务端反爬/压缩方式影响
        - 使用默认 SSL 上下文，提升 HTTPS 兼容性
        返回：解析后的 JSON 或 None
        """
        backoff = 0.6  # 初始退避（秒）
        last_exc = None
            ...
            try:
                req = urllib.request.Request(
                    ...
                    headers={
                        ...
                        # 仅声明 gzip，避免服务端返回 br 导致无法解压
                        "Accept-Encoding": "gzip",
                        # 主动关闭长连接，降低半关闭导致的远端断开
                        "Connection": "close",
                    },
                    method="GET",
                )
                ...
                with urllib.request.urlopen(req, timeout=15, context=context) as resp:
                    body = resp.read()
                    # 如果服务端返回 gzip 压缩内容，先解压
                    if "gzip" in (resp.headers.get("Content-Encoding", "").lower()):
                        import gzip
                        body = gzip.decompress(body)
                    encoding = resp.headers.get_content_charset() or "utf-8"
                    response_data = json.loads(body.decode(encoding, errors="replace"))
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"GET请求失败({type(e).__name__}): {e}. URL: {url}")
                    return None
            except Exception as e:
                print(f"GET请求异常: {e}. URL: {url}")
                return None
    ...
    def get_stock_label(self,stock_codes):
        ...
        for group in stock_groups:
            ...
            response_data = self.request_post(url, data, headers)
            if response_data:
            ...
            if data and 'data' in data and 'm{klt}'.format(klt=klt) in data['data'][stock_codes_str]:
                ...
                for item in klines:
                    parsed_data.append({
                                ...
                                '涨幅': round(pd.to_numeric(item[2], errors='coerce')/last_price*100-1,2),  #估算
                                '成交量': round(pd.to_numeric(item[5], errors='coerce')),
                                '成交额': round(pd.to_numeric(item[5], errors='coerce'))*pd.to_numeric(item[2], errors='coerce')  #估算
                            })
    ...
    def calc_raise_rate_new(self, stock_codes):
        ...
        for stock_code in stock_codes:
            ...
            if not df.empty:
                ...
                if min_close_ago > 0 and max_close_ago is not None:
