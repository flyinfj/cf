...
if current_time >= datetime.strptime('10:30:00', '%H:%M:%S').time():
    ...
    if current_time >= datetime.strptime('16:30:00', '%H:%M:%S').time():
        ...
        file_name2 = f'{myLib.MyLib().filedir_database}/tmp_stock_real.db'
        new_all_df2.to_csv(file_name2, mode='a', header=False, index=False)
        new_all_df2 = pd.read_csv(file_name2)
        new_all_df2 = new_all_df2[new_all_df2['时间'] > latest_60_data['trade_date'].iloc[-5]]
        new_all_df2.to_csv(file_name2, mode='w', header=True, index=False)
