...
def get_subjects(subject_id):
    ...
    if response.status_code == 200:
        data = response.json()
        res_file_name =  f'E:/python_workspace/cf/response/{subject_id}'
...
file_name =  f'E:/python_workspace/cf/data/{now.strftime("%m%d%H")}_messages.txt'
...
if message_df is not None:
    for index, item in message_df.iterrows():
        ...
        new_order = ['题材', '分类', '子类','代码', '名称','涨幅','理由']
