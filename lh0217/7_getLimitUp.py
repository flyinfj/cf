...
all_limitup_num = all_limitup_num[~(all_limitup_num['最早涨停日期']<=trade_dates.iloc[-6])]
all_limitup_num = all_limitup_num[~(all_limitup_num['最早涨停日期']>=trade_dates.iloc[-2])]
# 去掉涨停以来涨幅超过5%的股票
...
for index, row in all_limitup_num.iterrows():
    ...
    if code.startswith('60'):
        ...
        code = 'sz' + code
    limitup_date_str = row['最早涨停日期']
    limitup_date = datetime.strptime(row['最早涨停日期'], '%Y%m%d').strftime('%Y-%m-%d')
...
for index, row in all_limitup_num.iterrows():
    stock_code = row['代码']
    start_time = datetime.strptime(row['最早涨停日期'], '%Y%m%d')
    df = get_stock_his(stock_code, 5, start_time)
    total = df['成交量'].sum()
    result = {
        '代码': stock_code,
        '名称': row['名称'],
...
# 获取九转
start_date = datetime.strptime(trade_dates.iloc[0], "%Y%m%d")
...
#统计涨停次数
file_name = f"{model.filedir_database}/days_limitup.db"
df = pd.read_csv(file_name)
df['代码'] = df['代码'].astype(str).str.zfill(6)
count_df = df['代码'].value_counts().reset_index()
count_df.columns = ['代码', '涨停数']
