...
class MyLib:
    ...
    #获取交易日期列表(不含今日)
    def get_trade_dates(self, predays=5):
        ...
        df = df.tail(predays)
        df = df['trade_date'].apply(lambda x: x.strftime('%Y%m%d'))
        return df
    ...
    def get_cond_popu_stocks(self, topn):
        ...
        files = {
            ...
            'filter': (None, '(POPULARITY_RANK>=0)(POPULARITY_RANK<350)'),
            'p': (None, '1'),
            'ps': (None, topn),
            'sr': (None, '-1'),
            'st': (None, 'CHANGE_RATE'),
    ...
    def get_raise_stocks(self, topn):
        ...
        files = {
            'type': (None, 'RPTA_SECURITY_STOCKSELECT'),
            'sty': (None, 'SECUCODE,SECURITY_CODE,SECURITY_NAME_ABBR,NEW_PRICE,CHANGE_RATE,POPULARITY_RANK'),
            'filter': (None, '(CHANGE_RATE>=9)(CHANGE_RATE<=21)'),  # (@CHANGE_RATE="LIMIT_UP_PRICE")
            'p': (None, '1'),
            'ps': (None, topn),
            'sr': (None, '-1'),
            'st': (None, 'CHANGE_RATE'),
                ...
                if response_data and 'data' in response_data:
                    ...
                    for item in response_data['data']:
                        ...
                        parsed_data.append({
                            ...
                            '标签': label_names[:30].ljust(30) if len(label_names) > 30 else label_names.ljust(30)
                        })
    ...
    def get_stock_nineturn(self, stock_codes,start_date,klt=30,is_filter=0,is_name=0):
        ...
        for stock_code in stock_codes:
            ...
            if stock_his_df is not None and not stock_his_df.empty:
                ...
                # ud_df = ud_df[ud_df['cumu_ud'].astype(int).isin([-6,-7,-8,-9])]
                # ud_df['涨幅'] = ud_df['涨幅'].apply(lambda x: f"{x}%")
                if is_filter == 0:
                    ud_df = ud_df.tail(1)
                else:
                    t1 = ud_df[ud_df['cumu_ud'] >= 2]['时间'].max()
                    t2 = ud_df[ud_df['cumu_ud'] <= -7]['时间'].max()
        ...
        try:
            ...
            if hot_detail_df is not None and not hot_detail_df.empty and len(
                ...
                data = {
                    '代码': stock,  # 股票代码
                    '人气排名': avg_hot_1,
                    '排名变动': rc_raise_1,  # 排名变动
                    '排名变动3': rc_raise_3,  # 排名变动
                    '排名变动5': rc_raise_5  # 排名变动
                }
            else:
                data = {
                    '代码': stock,  # 股票代码
