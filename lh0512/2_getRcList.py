...
#获取今日热榜飙升前100+热榜350升1000/150升450/50升150
df = model.get_popu_stocks(1)
df = df[(((df['热榜'] < 350) & (df['热榜较昨日变动'] > 1000)) | ((df['热榜'] < 150) & (df['热榜较昨日变动'] > 450)) | ((df['热榜'] < 50) & (df['热榜较昨日变动'] > 150)))]
...
if df is not None and not df.empty:
    ...
    sorted_df = df_all.sort_values(by='热榜', ascending=True)
    sorted_df = sorted_df.drop(columns=['时间'])
    sorted_df = sorted_df.drop(columns=['收盘'])
    sorted_df = sorted_df.drop(columns=['最新价'])
    sorted_df = sorted_df.drop(columns=['成交量'])
    sorted_df = sorted_df.drop(columns=['成交额'])
    sorted_df = sorted_df[~sorted_df['名称'].str.contains('ST')]
    sorted_df.reset_index(drop=True, inplace=True)  # 删除索引列并重置索引
    print(model.beautify(sorted_df))
    model.output_file(sorted_df, '飙升个股信息(今日热榜飙升前100+热榜350升1000/150升450/50升150)')
