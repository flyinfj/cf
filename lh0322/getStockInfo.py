...
class GetStockInfo:
    ...
    def get_stock_close(self, stock_code):
        ...
        try:
            ...
            return pd.DataFrame()
    def get_limitup_sort(self,stockCodes):
        ...
        return pd.DataFrame(result, columns=['代码', '涨停排序'])
    ...
    def get_stock_info(self, stock_codes):
        ...
        limitup_sort_df = self.get_limitup_sort(stock_codes)
        merged_df = pd.merge(df, limitup_sort_df, on='代码', how='left')
        merged_df['股本'] = merged_df['股本'] + ' # ' + merged_df['涨停排序'].fillna('')
        merged_df.drop(columns=['涨停排序'], inplace=True)
        return merged_df
