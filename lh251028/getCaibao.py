...
if not df.empty:
    ...
    for i in range(0, len(stock_codes), batch_size):
        ...
        # 构建提示语
        prompt = f'''
        请根据以下信息，输出所有股票的代码CODE、2024年全年营收Y_R、2024年全年营收涨幅Y_RR、2024年全年净利润Y_P、2024年全年净利润涨幅Y_PR、2025年第一季度营收Q1_R、2025年第一季度营收涨幅Q1_RR、2025年第一季度净利润Q1_P、2025年第一季度净利润涨幅Q1_PR、2025年第二季度营收Q2_R、2025年第二季度营收涨幅Q2_RR、2025年第二季度净利润Q2_P、2025年第二季度净利润涨幅Q2_PR。
        说明:
        1. 无法获取的数据用 "0" 表示。
        ...
        {contents}
        '''
        result_df = model.get_chat(prompt)
    ...
    new_order = ['代码','名称','类型','公告时间','涨幅','Y_R', 'Y_RR', 'Y_P', 'Y_PR','Q1_R', 'Q1_RR', 'Q1_P', 'Q1_PR','Q2_R', 'Q2_RR', 'Q2_P', 'Q2_PR', '公告内容']
    df = df[new_order]
    df = df[(df["Q1_RR"] > 30) | (df["Q1_PR"] > 30) | (df["Q2_PR"] > 30) | (df["Q2_RR"] > 30)]
    ...
    df['Q2_RR'] = df['Q2_RR'].apply(lambda x: f"{x}%")
    df['Q2_PR'] = df['Q2_PR'].apply(lambda x: f"{x}%")

    df.rename(columns={'Y_R': 'Y收益'}, inplace=True)
    df.rename(columns={'Y_RR': 'Y收益比'}, inplace=True)
    df.rename(columns={'Y_P': 'Y利润'}, inplace=True)
    df.rename(columns={'Y_PR': 'Y利润比'}, inplace=True)
    df.rename(columns={'Q1_R': 'Q1收益'}, inplace=True)
    df.rename(columns={'Q1_RR': 'Q1收益比'}, inplace=True)
    df.rename(columns={'Q1_P': 'Q1利润'}, inplace=True)
    df.rename(columns={'Q1_PR': 'Q1利润比'}, inplace=True)
    df.rename(columns={'Q2_R': 'Q2收益'}, inplace=True)   
    df.rename(columns={'Q2_RR': 'Q2收益比'}, inplace=True)      
    df.rename(columns={'Q2_P': 'Q2利润'}, inplace=True) 
    df.rename(columns={'Q2_PR': 'Q2利润比'}, inplace=True)
    print(df)
